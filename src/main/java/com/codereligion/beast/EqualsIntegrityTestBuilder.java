...
/**
 * Builder for the equals integrity test. The resulting test will apply the following criteria to the class under test:
 * <p/>
 * <ul> <li> the equals method must be implemented <li> calling equals with {@code null} must always be false <li> calling equals on the same instance must
 * always be true <li> calling on two different instances must be symmetric <li> if included properties have been provided <ul> <li> the included properties
 * must be included in the implementation <li> all not included properties must not be included in the implementation </ul> <li> if excluded properties have
 * been provided <ul> <li> the excluded properties must be excluded from the implementation <li> all not excluded properties must be included in the
 ...
 * extending the equals implementation. This strategy aims for an accurate and complete equals implementation in order to cover as much of the class' state
 * variations as possible.
 * <p/>
 * <p/>
 * <b>When to use includes:</b> Use includes to make sure that no additional properties can be added to the equals implementation without altering the test.
 * This strategy aims for high-performance of the equals implementation, rather than for correctness. This strategy should only be used, when performance is
